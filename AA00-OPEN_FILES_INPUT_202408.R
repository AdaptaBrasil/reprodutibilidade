...
iMeta_in <- read.csv("INPUT/IMeta_RH_INDBRT.csv",sep=";",dec=".",header=TRUE,fileEncoding="latin1")
...
iData_input <- as.data.frame(read.csv("INPUT/IData_RH_INDBRT.csv",sep=";",dec=",",header=TRUE))
...
##### 
ADAPTA_coin <- new_coin(iData = iData_adapta, iMeta = iMeta_adapta)

coin <- qNormalise(ADAPTA_coin, dset = "Raw", f_n = "n_minmax")
